...
export function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Menu Overlay - Pro UI Balanced Design */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            <motion.div 
            ...
            <motion.div 
              ...
              dir="rtl"
            >
              {/* Drawer Header - Optimized Space */}
              <div className="flex-none p-5 border-b border-white/5 bg-muted/5 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-3">
                      <div className="w-11 h-11 bg-primary/10 rounded-xl flex items-center justify-center border border-primary/20 shadow-lg">
                         <Sparkles size={22} className="text-primary" />
                      </div>
                      <span className="font-black text-[12px] gold-text uppercase tracking-widest leading-tight">XMOOD<br/>SYSTEM</span>
                   </div>
                ...
                {user && (
                   <Link href="/wallet" className="flex items-center gap-4 p-3.5 rounded-2xl bg-card border border-primary/10 shadow-inner">
                      <Avatar className="w-11 h-11 border-2 border-primary/20">
                         <AvatarImage src={profile?.photoURL} />
                         <AvatarFallback className="bg-primary/5 text-primary font-bold text-xs">XM</AvatarFallback>
                      </Avatar>
              ...
              {/* Drawer Content - Pro Spacing */}
              <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-8">
                
                <div className="space-y-3">
                  <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.3em] pr-3 mb-3 opacity-50">التنقل الرئيسي</p>
                  <div className="grid gap-1.5">
                    {navLinks.map((link) => (
                      <Link 
                        key={link.href} 
                        href={link.href} 
                        className={`flex items-center justify-between p-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all ${pathname === link.href ? 'bg-primary text-black shadow-lg scale-[1.01]' : 'hover:bg-primary/5 text-foreground/80 bg-muted/10 border border-transparent'}`}
                      >
                ...
                <div className="space-y-4">
                   <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.3em] pr-3 mb-3 opacity-50">الأصول الرقمية</p>
                   <div className="grid grid-cols-1 gap-2">
                      {categories.length > 0 ? categories.map((cat, i) => (
                         <Link 
                           key={i} 
                           href={`/store?category=${cat}`} 
                           className="flex items-center gap-4 p-3.5 rounded-xl bg-muted/10 hover:bg-muted text-[10px] font-black text-foreground/80 border border-transparent hover:border-primary/10 transition-all"
                         >
                ...
                <div className="space-y-3 pb-4">
                   <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.3em] pr-3 mb-3 opacity-50">الأمان والضمان</p>
                   <Link href="/middleman" className="flex items-center gap-4 p-4 rounded-2xl bg-primary/5 border border-primary/10 group shadow-inner">
                   ...
                   {isAdmin && (
                    <Link 
                      href="/admin" 
                      className="flex items-center gap-4 p-4 rounded-2xl bg-blue-600/10 border border-blue-600/20 text-blue-500 shadow-inner mt-3"
                    >
                      <ShieldAlert size={22} />
                      <div className="flex flex-col text-right">
                         <span className="font-black text-[11px] uppercase tracking-widest">لوحة الإدارة السيادية</span>
                         <span className="text-[8px] opacity-60 font-black uppercase tracking-tighter mt-0.5">التحكم المركزي في النظام</span>
                      </div>
                    </Link>
                   )}
                </div>
              ...
              {/* Drawer Footer - Pro Balanced Layout */}
              <div className="flex-none p-5 border-t border-white/5 bg-muted/10 space-y-4">
                <div className="flex justify-between items-center bg-background/50 p-3 rounded-xl border border-white/5 shadow-inner">
                   <span className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pr-2">الوضع المظلم</span>
                   <div className="flex gap-4">
                      <button onClick={toggleTheme} className={`transition-all ${theme === 'light' ? 'text-primary scale-110' : 'opacity-30'}`}><Sun size={20} /></button>
                      <button onClick={toggleTheme} className={`transition-all ${theme === 'dark' ? 'text-primary scale-110' : 'opacity-30'}`}><Moon size={20} /></button>
                   </div>
                </div>

                {user ? (
                   <Button variant="ghost" onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="w-full justify-center gap-3 h-14 rounded-xl font-black text-[11px] text-red-500 bg-red-500/5 hover:bg-red-500 hover:text-white transition-all shadow-lg">
                      <LogOut size={18} /> تسجيل الخروج الآمن
                   </Button>
                ) : (
                  <Button asChild className="royal-button w-full h-14 text-[10px] shadow-2xl">
