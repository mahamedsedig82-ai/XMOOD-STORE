        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-8 md:mb-12">
          <Card className="luxury-card border-none p-6 md:p-8 lg:col-span-2 bg-white dark:bg-zinc-900 flex flex-col md:flex-row items-center gap-6 md:gap-8 shadow-xl">
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto">
               <Button asChild className="royal-button w-full h-11 md:h-12 text-[9px] md:text-xs">
                 <Link href="/wallet/transfer"><ArrowRightLeft size={14} className="ml-2" /> تحويل فوري</Link>
               </Button>
               <Button onClick={() => setIsEditing(true)} variant="outline" className="h-11 md:h-12 rounded-xl font-black text-[9px] md:text-[10px] uppercase gap-2 border-primary/20 hover:bg-primary/5">
                 <Edit2 size={12} /> تحديث الهوية
               </Button>
            </div>
          </Card>

          <Card className="luxury-card border-none p-6 md:p-8 bg-zinc-900 text-white dark:bg-zinc-900 flex flex-col justify-center text-center shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-8 md:mb-12">
           ...
           <Card className="luxury-card p-5 md:p-8 flex items-center gap-4 md:gap-8 border-none bg-card/60 backdrop-blur-xl shadow-xl">
              ...
              <div className="flex-1">
                 ...
                 <p className="text-[9px] md:text-xs text-zinc-500 dark:text-zinc-400 mb-3 font-medium">صلاحيات كاملة للموثقين.</p>
        ...
        <Card className="luxury-card border-none overflow-hidden bg-card/60 backdrop-blur-xl shadow-2xl">
          <CardHeader className="p-5 md:p-8 border-b flex flex-row items-center justify-between bg-muted/5">
            <CardTitle className="text-base md:text-xl font-black flex items-center gap-3">
              <History size={20} className="text-primary" /> {config?.walletPage?.ledgerTitle || "سجل العمليات"}
            </CardTitle>
            <Badge variant="outline" className="text-[7px] md:text-[9px] font-black uppercase tracking-widest border-primary/20 text-primary">Live Ledger</Badge>
          </CardHeader>
              ...
              <Table>
                <TableHeader className="bg-muted/30 sticky top-0 z-20">
                  <TableRow>
                    ...
                    <TableHead className="text-right font-black uppercase text-[9px]">المبلغ</TableHead>
                    <TableHead className="text-center font-black uppercase text-[9px]">القسيمة</TableHead>
                    <TableHead className="text-right font-black uppercase text-[9px] pr-8">التوقيت</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {transLoading ? (
                    ...
                    <TableRow><TableCell colSpan={5} className="text-center py-40 text-zinc-400 font-black uppercase tracking-widest opacity-40">لا توجد سجلات</TableCell></TableRow>
                  ) : transactions?.map((t: any) => (
                    <TableRow key={t.id} className="hover:bg-primary/5 border-b border-border/30 group">
                      <TableCell className="py-4 pr-8" data-label="العملية">
                        <div className="flex flex-col">
                           <span className="font-bold text-xs">{t.description}</span>
                           {t.orderId && (
                             <span onClick={() => copyToClipboard(t.orderId, "REF")} className="text-[7px] font-mono text-primary uppercase tracking-tighter mt-1 cursor-pointer hover:underline flex items-center gap-1">REF: {t.orderId.substring(0,8)}... <Copy size={8} /></span>
                           )}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-center" data-label="القسيمة">
                         {t.orderId ? (
                           <Button asChild variant="ghost" size="icon" className="h-9 w-9 rounded-lg hover:bg-primary/10 text-primary">
                              <Link href={`/orders/${t.orderId}`}><Eye size={16} /></Link>
                           </Button>
                         ) : <span className="text-[10px] text-zinc-400">---</span>}
                      </TableCell>
                      <TableCell data-label="التوقيت" className="text-zinc-500 text-[8px] md:text-[10px] font-black uppercase pr-8">
         ...
         <DialogContent className="max-w-md bg-card border-none rounded-[2rem] p-6 md:p-10 shadow-2xl">
            ...
            <div className="space-y-6 mt-6">
               ...
               <div className="flex flex-col items-center gap-4">
                  <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                     <Avatar className="w-20 h-20 md:w-24 md:h-24 border-4 border-primary/20 shadow-xl rounded-2xl overflow-hidden">
               ...
               <div className="space-y-4">
                  <div className="space-y-2">
                     <Label className="text-[9px] font-black uppercase text-primary pr-2">اسم العرض</Label>
                  ...
                  <div className="space-y-2">
                     <Label className="text-[9px] font-black uppercase text-primary pr-2">رقم الهاتف</Label>
            ...
            <DialogFooter className="mt-8">
               <Button onClick={handleUpdateProfile} disabled={isUpdating} className="royal-button w-full h-14 text-base">
