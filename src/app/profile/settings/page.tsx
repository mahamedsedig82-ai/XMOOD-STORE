...
import { Loader2, Save, User, Camera, Phone, MapPin, Calendar, Smartphone, Zap } from "lucide-react";
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        setPhotoURL(b64);
        toast({ title: "تم تحديث الصورة مؤقتاً، يرجى الحفظ" });
      } catch (err) {
    ...
    <main className="min-h-screen bg-background pb-32" dir="rtl">
      ...
      <div className="container mx-auto px-4 pt-32 max-w-4xl text-right">
        <header className="mb-12">
           ...
           <p className="text-muted-foreground mt-2 font-bold uppercase tracking-widest text-[10px]">Personal Identity & Sovereign Settings</p>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
           <div className="lg:col-span-1">
              <Card className="luxury-card p-10 border-none bg-primary/5 flex flex-col items-center gap-8 shadow-xl">
                 <div className="relative group">
                    <Avatar className="w-40 h-40 md:w-48 md:h-48 rounded-[2.5rem] border-4 border-background shadow-2xl overflow-hidden transition-transform duration-500 group-hover:scale-105">
                    ...
                    <button 
                      ...
                      className="absolute bottom-4 right-4 bg-primary text-black p-3 rounded-2xl shadow-xl hover:scale-110 transition-all border-4 border-background"
                    >
                       <Camera size={20} />
                    </button>
                 ...
                 <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="space-y-2">
                          ...
                          <Input value={form.residence} onChange={e => setForm({...form, residence: e.target.value})} placeholder="مثال: الخرطوم، حي الرياض" />
                       </div>
                       <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                       ...
                       <Textarea value={form.bio} onChange={e => setForm({...form, bio: e.target.value})} placeholder="اكتب شيئاً عن نفسك يراه المبدعون والوكلاء..." className="min-h-[120px]" />
                    </div>
                    
                    <div className="pt-8 flex gap-4">
                       <Button onClick={handleSave} disabled={isSaving} className="royal-button flex-1 h-18 text-xl shadow-xl">
