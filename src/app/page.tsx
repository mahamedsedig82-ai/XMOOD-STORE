  ...
  return (
    <main className="min-h-screen bg-background" dir="rtl">
      ...
      {products && products.length > 0 && (
        <section className="py-24 md:py-32 bg-muted/10 border-y">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {[
                ...
                <div key={i} className="p-10 luxury-card bg-card/40 border-primary/10 hover:border-primary/25 shadow-xl text-center flex flex-col items-center group transition-all duration-500">
                   <div className="w-16 h-16 bg-primary/5 rounded-[1.5rem] flex items-center justify-center text-primary mb-8 group-hover:bg-primary group-hover:text-black transition-all shadow-inner">
                      <item.icon size={32} />
                   </div>
                   <h4 className="font-black text-xl mb-4 group-hover:gold-text transition-all tracking-tight">{item.title}</h4>
                   <p className="text-muted-foreground leading-relaxed font-medium text-sm opacity-80">{item.desc}</p>
                </div>
              ))}
           </div>
        </div>
