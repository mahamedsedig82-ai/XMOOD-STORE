...
import { Printer, Download, CheckCircle2, Clock, ShieldCheck, Mail, Tag, Truck, Zap, Loader2 } from "lucide-react";
...
export default function OrderInvoicePage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-primary" size={60} /></div>;
  if (!order) return <div className="min-h-screen flex items-center justify-center text-3xl font-black">القسيمة غير موجودة</div>;
    ...
    <main className="min-h-screen bg-muted/30 pb-40" dir="rtl">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
           <Card id="invoice-render" className="luxury-card border-none bg-white dark:bg-zinc-950 p-10 md:p-16 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-12 relative z-10 border-b pb-12 mb-12">
                 <div className="space-y-4">
                    ...
                    <div className="pt-4 space-y-2">
                       <p className="text-xs font-bold flex items-center gap-3"><Tag size={14} className="text-primary" /> مرجع العملية: <span className="font-mono text-primary">{order.id}</span></p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-16 relative z-10">
                 <div>
                    <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-6 flex items-center gap-2"><ShieldCheck size={14} /> بيانات المستحوذ</h4>
                    <p className="text-xl font-black">{order.userName}</p>
                    <p className="text-sm text-muted-foreground mt-2">{order.userEmail}</p>
                    <p className="text-[9px] font-mono text-zinc-500 mt-2">UID: {order.userId}</p>
                 </div>
                 <div className="md:text-left">
                    <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-6 flex items-center gap-2 justify-end"><Truck size={14} /> لوجستيات التسليم</h4>
                    <p className="text-xl font-black">{order.shippingMethodName}</p>
                    <p className="text-sm text-muted-foreground mt-2">البريد المعتمد: {order.deliveryEmail || "غير محدد"}</p>
                 </div>
              </div>

              <div className="relative z-10 space-y-6">
                 <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-6">الأصول والخدمات المشمولة</h4>
              ...
              <div className="mt-16 flex flex-col md:flex-row justify-between items-end gap-12 relative z-10">
                 <div className="flex-1 w-full md:w-auto">
                    {order.shippingCodeSent && (
                      <div className="p-8 bg-primary/5 border-2 border-dashed border-primary/30 rounded-[2rem]">
                         <p className="text-[9px] font-black text-primary uppercase mb-4 tracking-widest">كود التفعيل / الشحن</p>
                         <p className="text-3xl font-black tracking-[0.2em] gold-text select-all">{order.shippingCodeSent}</p>
                      </div>
                    )}
                    {order.notes && (
                      <div className="mt-8">
                         <p className="text-[9px] font-black text-muted-foreground uppercase mb-2">ملاحظات العميل</p>
                         <p className="text-xs font-medium italic opacity-70">"{order.notes}"</p>
                      </div>
                    )}
                 </div>
                 <div className="w-full md:w-80 space-y-4">
                    ...
                    <div className="flex justify-between items-end">
                       <span className="font-black text-sm uppercase">صافي القيمة</span>
                       <span className="text-5xl font-black gold-text tracking-tighter">{formatUSD(order.totalAmount)}</span>
                    </div>
                 </div>
              </div>

              <div className="mt-20 pt-10 border-t text-center space-y-4 opacity-40 grayscale">
                 <p className="text-[8px] font-black uppercase tracking-[0.5em]">Generated via XMOOD Sovereign Cloud Settlement Engine</p>
      ...
      <style jsx global>{`
        @media print {
          ...
          #invoice-render { box-shadow: none !important; border: 1px solid #eee !important; }
        }
      `}</style>
    </main>
  );
