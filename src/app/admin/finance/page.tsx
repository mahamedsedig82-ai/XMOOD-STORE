...
import { collection, query, orderBy, limit, doc, updateDoc, addDoc, where, getDocs, serverTimestamp } from "firebase/firestore";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Wallet, Search, RefreshCw, ArrowUpRight, ArrowDownLeft, Loader2, History, Banknote, Zap, Clock, DollarSign } from "lucide-react";
...
export default function AdminFinanceCentralBank() {
  ...
  const handleUpdateRate = () => {
    ...
    updateDoc(settingsRef, {
      ...
      toast({ title: "تم تحديث سعر الصرف", description: "سيتم تطبيق السعر الجديد على كافة أرجاء الموقع." });
      setNewRate("");
    }).catch(async (e) => {
  ...
  return (
    <div className="space-y-12 animate-fade-in" dir="rtl">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-5xl font-headline font-bold gold-text">الإدارة المالية المركزية</h1>
          <p className="text-muted-foreground mt-2 font-bold uppercase tracking-widest text-xs italic">Global Monetary Control Hub</p>
        </div>
        <div className="flex gap-4">
           <Badge variant="outline" className="border-green-500/20 text-green-500 px-6 py-2 rounded-full font-bold">SYSTEM ACTIVE</Badge>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <Card className="luxury-card border-none bg-primary/5">
            <CardHeader className="p-8 border-b border-primary/10">
              <CardTitle className="flex items-center gap-4 text-xl font-bold">
                <DollarSign className="text-primary" /> سعر الدولار اليوم
              </CardTitle>
            </CardHeader>
              ...
              {targetUser && (
                <div className="p-6 bg-primary/5 rounded-[2rem] border border-primary/10 space-y-6 animate-fade-up">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Button onClick={() => handleUpdateBalance('deposit')} disabled={isProcessing} className="bg-green-600 hover:bg-green-700 text-white font-black h-12 rounded-xl shadow-lg">إيداع</Button>
                    <Button onClick={() => handleUpdateBalance('withdrawal')} disabled={isProcessing} variant="destructive" className="font-black h-12 rounded-xl shadow-lg">خصم</Button>
                  </div>
                </div>
              )}
            </CardContent>
        ...
        <Card className="luxury-card border-none lg:col-span-2 overflow-hidden bg-card/60 backdrop-blur-xl">
          <CardHeader className="p-8 border-b flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-4 text-xl font-bold">
              <History className="text-primary" /> سجل التدفقات المالية
            </CardTitle>
            <Badge variant="outline" className="text-muted-foreground uppercase text-[8px] font-black px-4">Live Ledger</Badge>
          </CardHeader>
              ...
              <Table className="responsive-table">
                ...
                <TableBody>
                  {transLoading ? (
                    ...
                    <TableRow><TableCell colSpan={4} className="text-center py-40 font-bold opacity-30">لا توجد سجلات بعد</TableCell></TableRow>
                    ...
                    <TableRow key={t.id} className="hover:bg-primary/5 border-border transition-all">
                      ...
                      <TableCell className={`text-left pl-10 font-black text-xl ${t.type === 'deposit' ? 'text-green-500' : 'text-red-500'}`} data-label="المبلغ">
