...
export default function AdminContentManager() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center p-40 gap-6">
       ...
       <p className="text-[10px] font-black uppercase tracking-widest gold-text">Initializing Identity Master...</p>
    </div>
  );
    ...
    <div className="space-y-12 animate-fade-in pb-32 text-right" dir="rtl">
      ...
      <Tabs defaultValue="visual" className="w-full">
        ...
        <TabsContent value="visual">
           <Card className="luxury-card p-8 md:p-12 space-y-10 border-none bg-card/40 shadow-2xl">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                 <div className="space-y-8">
                    <div className="space-y-4">
                       ...
                       >
                          {form.appearance.logoUrl ? (
                            <img src={form.appearance.logoUrl} className="h-full w-full object-contain p-8" style={{ borderRadius: '2.5rem' }} alt="Logo Preview" />
                          ) : (
                            <div className="text-center space-y-3">
                               <Upload className="text-primary mx-auto" size={48} />
                               <p className="text-[10px] font-black uppercase text-muted-foreground opacity-40">انقر لاختيار صورة من هاتفك</p>
                            </div>
                          )}
                    ...
                    <div className="space-y-4">
                       <Label className="text-[10px] font-black uppercase text-muted-foreground pr-4">لون الهوية الأساسي</Label>
                 ...
                 <div className="bg-muted/20 rounded-[3.5rem] p-10 flex flex-col items-center justify-center border-2 border-dashed border-primary/20">
                    <h4 className="text-xs font-black uppercase tracking-[0.4em] text-primary mb-12">معاينة العلامة التجارية</h4>
                    <div className="p-16 bg-card rounded-[3.5rem] shadow-[0_0_80px_rgba(0,0,0,0.1)] border flex flex-col items-center gap-8 min-w-[320px]">
                       <div className="h-24 w-auto flex items-center justify-center">
                          {form.appearance.logoUrl ? (
                            ...
                            <span className="handwritten-logo text-5xl">XMOOD <span>Store</span></span>
                          )}
                       </div>
                       <div className="h-2 bg-muted rounded-full w-full opacity-20" />
                       <div className="h-2 bg-muted rounded-full w-2/3 opacity-20" />
                       <Button style={{ backgroundColor: form.appearance.primaryColor }} className="w-full h-14 rounded-2xl text-white font-black text-[10px] uppercase shadow-lg">Action Preview</Button>
                    </div>
                 </div>
              </div>
           </Card>
        ...
        <TabsContent value="site">
           <Card className="luxury-card p-12 space-y-12 border-none bg-primary/5 shadow-2xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                 ...
                 <div className="space-y-3">
                    <Label className="text-[10px] font-black uppercase text-muted-foreground pr-4 flex items-center gap-2"><DollarSign size={14}/> سعر صرف الدولار (SDG)</Label>
        ...
        <TabsContent value="sections">
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <Card className="luxury-card p-10 space-y-8 bg-card/60 shadow-2xl">
                 <h3 className="text-2xl font-black flex items-center gap-4 text-primary"><LayoutGrid size={28} /> نصوص معرض الإبداع</h3>
                 <div className="space-y-6">
                    <div className="space-y-2"><Label className="text-[9px] font-black uppercase opacity-60">العنوان الرئيسي</Label><Input value={form.gallerySettings.title} onChange={e => setForm({...form, gallerySettings: {...form.gallerySettings, title: e.target.value}})} /></div>
                    <div className="space-y-2"><Label className="text-[9px] font-black uppercase opacity-60">نص الزر</Label><Input value={form.gallerySettings.buttonText} onChange={e => setForm({...form, gallerySettings: {...form.gallerySettings, buttonText: e.target.value}})} /></div>
                    <div className="space-y-2"><Label className="text-[9px] font-black uppercase opacity-60">الوصف الفرعي</Label><Textarea value={form.gallerySettings.subtitle} onChange={e => setForm({...form, gallerySettings: {...form.gallerySettings, subtitle: e.target.value}})} /></div>
                 </div>
              </Card>
              <Card className="luxury-card p-10 space-y-8 bg-zinc-950/40 shadow-2xl text-white">
                 <h3 className="text-2xl font-black flex items-center gap-4 gold-text"><ShieldCheck size={28} /> نصوص الوكلاء والسلة</h3>
                 <div className="space-y-6">
                    <div className="space-y-2"><Label className="text-[9px] font-black uppercase text-primary">عنوان صفحة الوكلاء</Label><Input value={form.agentSettings.title} onChange={e => setForm({...form, agentSettings: {...form.agentSettings, title: e.target.value}})} className="bg-zinc-900" /></div>
                    <div className="space-y-2"><Label className="text-[9px] font-black uppercase text-primary">عنوان السلة الرئيسي</Label><Input value={form.cartLabels.cartTitle} onChange={e => setForm({...form, cartLabels: {...form.cartLabels, cartTitle: e.target.value}})} className="bg-zinc-900" /></div>
                    <div className="space-y-2"><Label className="text-[9px] font-black uppercase text-primary">رسالة النجاح</Label><Input value={form.cartLabels.successMsg} onChange={e => setForm({...form, cartLabels: {...form.cartLabels, successMsg: e.target.value}})} className="bg-zinc-900" /></div>
                 </div>
              </Card>
           </div>
        </TabsContent>
        ...
        <TabsContent value="footer">
           <Card className="luxury-card p-12 space-y-12 border-none bg-zinc-950 text-white shadow-2xl">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                 <div className="space-y-3"><Label className="text-[10px] font-black uppercase text-primary">رقم الواتساب</Label><Input value={form.contact.whatsapp} onChange={e => setForm({...form, contact: {...form.contact, whatsapp: e.target.value}})} className="bg-white/5 border-primary/20 text-center" /></div>
                 <div className="space-y-3"><Label className="text-[10px] font-black uppercase text-primary">رابط تيليجرام</Label><Input value={form.contact.telegram} onChange={e => setForm({...form, contact: {...form.contact, telegram: e.target.value}})} className="bg-white/5 border-primary/20 text-center" /></div>
                 <div className="space-y-3"><Label className="text-[10px] font-black uppercase text-primary">انستجرام</Label><Input value={form.contact.instagram} onChange={e => setForm({...form, contact: {...form.contact, instagram: e.target.value}})} className="bg-white/5 border-primary/20 text-center" /></div>
              </div>
              <div className="pt-12 border-t border-white/10 space-y-10">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-3"><Label className="text-[10px] font-black uppercase opacity-60">نص "عن المتجر" في التذييل</Label><Textarea value={form.footer.aboutText} onChange={e => setForm({...form, footer: {...form.footer, aboutText: e.target.value}})} className="bg-white/5 border-primary/10 rounded-[2rem]" /></div>
                    <div className="space-y-3"><Label className="text-[10px] font-black uppercase opacity-60">حقوق الملكية</Label><Input value={form.footer.copyright} onChange={e => setForm({...form, footer: {...form.footer, copyright: e.target.value}})} className="bg-white/5 border-primary/10" /></div>
                 </div>
              </div>
           </Card>
        </TabsContent>
