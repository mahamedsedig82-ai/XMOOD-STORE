...
export default function DesignerPortfolioAdmin() {
  ...
  const handleAddToGallery = async () => {
    if (!newDesign.title || !newDesign.imageUrl || !db || !user) return;
    setIsProcessing(true);
    try {
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-8 bg-card/60 p-8 rounded-[2.5rem] border shadow-sm">
        ...
        <Dialog open={isGalleryOpen} onOpenChange={setIsGalleryOpen}>
          ...
          <DialogContent className="bg-card border-none rounded-[3rem] p-10 max-w-2xl overflow-y-auto max-h-[90vh]">
            <DialogHeader><DialogTitle className="text-3xl font-black gold-text flex items-center gap-4"><ImageIcon size={28} /> إضافة لتصاميم النخبة</DialogTitle></DialogHeader>
          ...
          <Card key={item.id} className="luxury-card border-none flex flex-col group h-full shadow-lg">
             ...
             <div className="p-5 bg-muted/30 border-t mt-auto">
                <Button 
                  ...
                  variant="destructive" 
                  className="w-full h-12 rounded-xl gap-3 font-black text-[10px] uppercase tracking-widest shadow-xl shadow-red-500/10"
                >
                   {isProcessing ? <Loader2 className="animate-spin w-4 h-4" /> : <><Trash2 size={18} /> حذف العمل الفني نهائياً</>}
                </Button>
             </div>
          </Card>
        ))}
