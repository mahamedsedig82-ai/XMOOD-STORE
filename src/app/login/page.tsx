...
export default function SecureLoginPage() {
  ...
  // === التقاط نتيجة إعادة التوجيه من جوجل (الحل الجذري) ===
  useEffect(() => {
    if (!auth || !db) return;
    ...
    const checkRedirect = async () => {
      try {
        setLoading(true);
        console.log("[AUTH-PHASE] Checking for redirect result...");
        const result = await getRedirectResult(auth);
        
        if (result?.user) {
          console.log("[AUTH-PHASE] Redirect success for:", result.user.email);
          const user = result.user;
          const userDocRef = doc(db, "users", user.uid);
          const userDoc = await getDoc(userDocRef);

          // إنشاء الملف الشخصي فوراً إذا لم يكن موجوداً
          if (!userDoc.exists()) {
            console.log("[AUTH-PHASE] Initializing new profile...");
        ...
        console.error("[AUTH-PHASE-ERROR] Redirect processing failed:", error);
        toast({ variant: "destructive", title: "فشل الدخول", description: "حدث خطأ أثناء معالجة بيانات جوجل." });
        setLoading(false);
      }
    };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      console.log("[AUTH-PHASE] Initiating Google Redirect...");
      const provider = new GoogleAuthProvider();
      provider.setCustomParameters({ prompt: 'select_account' });
      // الانتقال لنظام التوجيه لضمان استقرار العملية 100%
      await signInWithRedirect(auth, provider);
    } catch (error: any) {
      console.error("[AUTH-PHASE-ERROR] Redirect Init Error:", error);
