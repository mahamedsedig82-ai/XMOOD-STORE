...
export default function MarketplacePage() {
  ...
  const handleCreateListing = async () => {
    ...
    try {
      ...
      toast({ title: "تم نشر الإعلان الرسمي", description: "عرضك متاح الآن في السوق الموحد." });
    } catch (e) {
      ...
      <section className="py-32 relative overflow-hidden bg-black border-b border-primary/10">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
           <Badge className="mb-8 bg-primary/20 text-primary border-primary/40 py-2 px-10 rounded-full font-black tracking-[0.6em] uppercase text-[10px]">
              Sovereign Social Marketplace PRO
           </Badge>
           ...
           <p className="text-xl md:text-2xl text-slate-500 max-w-4xl mx-auto leading-relaxed font-light">
             تواصل، تداول، وابنِ هويتك السيادية في أول مجتمع تجاري موثق.
           </p>
        </div>
      </section>
        ...
        <Tabs defaultValue="listings" className="space-y-16">
          <div className="flex justify-center">
            <TabsList className="bg-white/5 backdrop-blur-3xl p-1.5 rounded-full h-20 shadow-2xl border border-white/10 flex gap-2 w-fit">
              <TabsTrigger value="listings" className="rounded-full px-12 font-black text-sm data-[state=active]:bg-primary data-[state=active]:text-black transition-all">الإعلانات</TabsTrigger>
              <TabsTrigger value="leaderboard" className="rounded-full px-12 font-black text-sm data-[state=active]:bg-primary data-[state=active]:text-black transition-all">التصنيف السيادي</TabsTrigger>
              <TabsTrigger value="escrow" className="rounded-full px-12 font-black text-sm data-[state=active]:bg-primary data-[state=active]:text-black transition-all">نظام الضمان</TabsTrigger>
            </TabsList>
          </div>
          ...
          <TabsContent value="listings" className="animate-fade-in space-y-12">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
               <div className="relative w-full md:w-96">
                  ...
                  <Input placeholder="البحث في السوق..." className="pr-12 h-14 bg-white/5 border-none rounded-2xl text-white shadow-inner" />
               </div>
               <Dialog open={isListingOpen} onOpenChange={setIsListingOpen}>
                  ...
                  <DialogContent className="bg-zinc-950 border-primary/20 rounded-[3rem] p-12 text-white shadow-2xl">
                    <DialogHeader>
                      <DialogTitle className="text-3xl font-bold gold-text flex items-center gap-4"><Megaphone size={28} /> إنشاء إعلان رسمي</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-6 mt-10">
                       <Input placeholder="عنوان الإعلان" className="h-14 bg-zinc-900 border-none rounded-xl" value={listingForm.title} onChange={e => setListingForm({...listingForm, title: e.target.value})} />
                       <Textarea placeholder="تفاصيل العرض..." className="min-h-[150px] bg-zinc-900 border-none rounded-2xl p-6" value={listingForm.description} onChange={e => setListingForm({...listingForm, description: e.target.value})} />
                       <div className="grid grid-cols-2 gap-4">
                          <Input type="number" placeholder="المبلغ" className="h-14 bg-zinc-900 border-none rounded-xl" value={listingForm.price} onChange={e => setListingForm({...listingForm, price: e.target.value})} />
          ...
          <TabsContent value="leaderboard" className="space-y-16 animate-fade-in">
             <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
                <Card className="lg:col-span-1 luxury-card border-none p-10 legendary-border">
                   <h3 className="text-xs font-black uppercase tracking-[0.4em] text-primary mb-10 italic">هويتك الاجتماعية</h3>
                   {user ? (
                     ...
                     <div className="text-center py-20 space-y-6">
                        ...
                        <p className="text-zinc-500 font-bold">يرجى تسجيل الدخول للوصول للرتب الاجتماعية.</p>
                     </div>
                   )}
                </Card>
                ...
                <Card className="lg:col-span-3 luxury-card border-none p-12">
                   <div className="flex justify-between items-center mb-12 flex-row-reverse">
                      <h3 className="text-3xl font-bold flex items-center gap-6 gold-text">
                        <Trophy size={36} className="text-primary" /> تصنيف نخبة XMOOD PRO
                      </h3>
                      <Badge variant="outline" className="border-primary/20 text-primary py-2 px-8 rounded-full font-black text-[9px]">GLOBAL LIVE RANKING</Badge>
                   </div>
                   <div className="space-y-4">
                      {marketplaceUsers?.map((u: any, i: number) => (
                        <div key={u.id} className="flex items-center justify-between p-6 bg-white/5 rounded-[2rem] border border-white/5 group hover:bg-primary/5 transition-all flex-row-reverse">
                           <div className="flex items-center gap-6 flex-row-reverse">
                              ...
                              <div className="text-right">
                                 <p className="font-bold text-lg mb-1 flex items-center gap-2 justify-end">
                                    {u.displayName}
          ...
          <TabsContent value="escrow" className="animate-fade-in">
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <Card className="luxury-card border-none p-16 legendary-border">
                  <h3 className="text-3xl font-bold mb-12 flex items-center gap-6 gold-text flex-row-reverse">
                    <ArrowLeftRight size={36} className="text-primary" /> نظام الضمان السيادي
                  </h3>
                  <div className="space-y-12">
                    <div className="space-y-4 text-center">
                      <p className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em]">سيولتك الموثقة</p>
                      <p className="text-6xl font-black text-white tracking-tighter">{formatUSD(profile?.walletBalance || 0)}</p>
                    </div>
                    <div className="space-y-6">
                       <Input placeholder="أدخل معرف المستلم (UID)" className="h-20 bg-white/5 border-none rounded-[2rem] text-2xl font-black text-center text-primary shadow-inner" />
                ...
                <div className="grid grid-cols-1 gap-10">
                   <Card className="luxury-card border-none p-12 bg-white/5">
                      <h4 className="text-2xl font-bold mb-6 flex items-center gap-4 text-primary justify-end">
                        حماية النواة PRO <ShieldCheck size={28} />
                      </h4>
                      <p className="text-zinc-400 text-lg leading-relaxed font-light text-right">
                        تتم كافة العمليات تحت رقابة النواة الذكية. لا يتم تحرير الأرصدة إلا بعد تأكيد استلام الأصول الرقمية المشتراة.
                      </p>
                   </Card>
                   <Card className="luxury-card border-none p-12 bg-white/5">
                      <h4 className="text-2xl font-bold mb-6 flex items-center gap-4 text-primary justify-end">
                        التوثيق الرسمي <BadgeCheck size={28} />
                      </h4>
                      <p className="text-zinc-400 text-lg leading-relaxed font-light text-right">
                        احصل على "شارة الموثوقية" عبر زيادة عمليات التبادل الناجحة ورفع نقاط أفضليتك في المجتمع.
                      </p>
                   </Card>
                </div>
             </div>
